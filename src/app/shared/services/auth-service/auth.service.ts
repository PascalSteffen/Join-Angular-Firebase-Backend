...
export class AuthService {
  ...
  SignIn(email: string, password: string) {
    return this.afAuth
      ...
      .catch((error) => {
        this.utilityService.alert(error, 5000);
        setTimeout(() => {
          console.clear();
        }, 1);
      });
  ...
  ForgotPassword(passwordResetEmail: string) {
    return this.afAuth
      ...
      .then(() => {
        this.utilityService.alert('Done! Check your E-Mails.', 5000);
      })
      .catch((error) => {
        this.utilityService.alert(error, 5000);
        setTimeout(() => {
   ...
   * @param displayName
   */
  updateUser(displayName: string, email: string) {
    const auth = getAuth();
    ...
    updateEmail(auth.currentUser, email).then(() => {
      this.SendVerificationMail();
      this.utilityService.alert('Profile updated! If you have changed your email, please check your emails and confirm the verification.', 5000);
    })
  }
